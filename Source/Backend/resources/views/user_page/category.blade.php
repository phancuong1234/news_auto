...
@section('page') {{ $idCate->name_category }}
@endsection
@section('url-page') {{ url('danh-muc/'.$idCate->slug) }}
@endsection
@section('content')
    <div class="main">
            ...
            <div class="row">
                <div class="main__content col-xl-8  border-right" id="main-content">
                    <div class="content__card row mb-4">
                        <a class="card__image img-full col-md-6 mb-2 mb-md-0" href="{{route('detail', [$main_news_cate->slug_cate, $main_news_cate->slug])}}"><img class="img-main img-news-cate-big lazyload lazy" data-src="{{ (strpos($main_news_cate['image'], config('setting.type_img.img_of_serve')) === 0)?asset('images/news/'.$main_news_cate['image']):$main_news_cate['image'] }}" alt="image-9"></a>
                    ...
                    <div class="row">
                        @foreach($news_cate as $key)
                            <div class="media col-md-6 mb-3">
                                <a href="{{route('detail', [$key->slug_cate, $key->slug])}}" class="img-news-home"><img class="mr-3 lazyload lazy" data-src="{{ (strpos($key->image, config('setting.type_img.img_of_serve')) === 0)?asset('images/news/'.$key->image):$key->image }}" alt="image-4"></a>
                    ...
                    <div class="container mt-5 pt-5 pl-0 border-top border-success">
                        @foreach($all_news_cate as $key)
                            <div class="row media mb-3">
                                <a class="cate__img col-12 col-md-4 w100" href="{{ route('detail', [$key->slug_cate, $key->slug]) }}"><img class="img-news img-news-cate-md lazyload lazy"  data-src="{{ (strpos($key->image, config('setting.type_img.img_of_serve')) === 0)?asset('images/news/'.$key->image):$key->image }}" alt="category1"></a>
